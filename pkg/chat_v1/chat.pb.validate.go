...
func (m *Message) validate(all bool) error {
	...
	if l := utf8.RuneCountInString(m.GetFrom()); l < 1 || l > 32 {
	...
	if l := utf8.RuneCountInString(m.GetText()); l < 50 || l > 500 {
		err := MessageValidationError{
			field:  "Text",
			reason: "value length must be between 50 and 500 runes, inclusive",
		}
		if !all {
			return err
		}
...
func (m *Users) validate(all bool) error {
	...
	if l := len(m.GetUsernames()); l < 2 || l > 30 {
		err := UsersValidationError{
			field:  "Usernames",
			reason: "value must contain between 2 and 30 items, inclusive",
		}
		if !all {
			return err
		}
